function compileMeasurements_gridsearch (in_folderOpfronted, out_folderResults)

    in_logfile = [out_folderResults '/compileMeasurements.log'];
    diary(in_logfile)
    diary ON
    ...
    % ----- load configurations -----------
    baseDataDir               	= '/scratch/agarcia/Tests/ValidationPhantom_IMALIFE/'

    cfg.files.folderCT         	= [baseDataDir '/Volume/'];
    cfg.files.folderOpfronted  	= in_folderOpfronted;
    cfg.files.saveFileResults	= [out_folderResults '/Opfront_ResultsPerBranch.csv'];
    ...
    for dd = 1:numel(fileList)
        ...
        files.airways_wall        = [files.folderOpfronted files.rootName '_outer.csv'];
