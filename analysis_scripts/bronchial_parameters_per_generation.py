...
def plot_loa(x, y, generation):
    ...
    text = f"$R^{2}$ = {r2:.3f}"
    ax2.text(0.05, 0.95, text, transform=ax2.transAxes, fontsize=14,
             verticalalignment='top', bbox=props)
    fig.tight_layout()
    fig.savefig(str(Path(outdir / f"bland_altmann_ping_{param}_{generation}.jpg").resolve()), dpi=300)
    ...
    for index, child_dir in enumerate(m_dir.iterdir()):
        ...
        for file_path in child_dir.iterdir():
            ...
            if param == 'pi10':
                ...
                try:
                    wap_df = tree.groupby('generation')['wall_global_area_perc'].describe()
                ...
                try:
                    ai_df = tree.groupby('generation')['inner_global_area'].describe()
                    res = ai_df.at[gen, 'mean']
                except (KeyError, TypeError) as e:
                    logging.error(f"Error processing ai for {result_number}\n {e}")
            elif param == 'count':
                try:
                    count_df = tree.groupby('generation')['wall_global_thickness'].describe()
                    res = count_df['count'].to_numpy().tolist()
                except (KeyError, TypeError) as e:
                    logging.error(f"Error processing count for {result_number}\n {e}")
...
def main():
    ...
    for generation in in_args.generation:
        ...
        for dir_1 in iter_dir:
            ...
            pool = Pool(4)
            results, errors = zip(*pool.starmap(calculate_parameters, zip(dirs, params, outdirs, generations)))
        ...
        if param == 'pi10':
            ...
            columns_csv = ['ID', 'first_vol', f'WAP_{generation}_first', 'second_vol', f'WAP_{generation}_repeat']
        elif param == 'ai':
            columns_csv = ['ID', 'first_vol', f'AI_{generation}_first', 'second_vol', f'AI_{generation}_repeat']
        elif param == 'count':
            columns_csv = ['ID', 'first_vol', f'count_{generation}_first', 'second_vol', f'count_{generation}_repeat']
        else:
...
if __name__ == "__main__":
    ...
    aparse.add_argument("param", type=str, help="Bronchial Parameter to summarise. \n Options: WA%, Ai, Pi10.",
                        default="Pi10")
    aparse.add_argument("generation", nargs="+", type=int, help="Generations to summarise.")
    ...
    logging.basicConfig(
        filename=str(Path(outdir / f"error_log_{datetime.now().strftime('%d_%m_%H_%M')}.txt").resolve()))
    if in_args.debug:
