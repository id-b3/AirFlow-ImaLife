    ...
    for directory in main_path.iterdir():
        ...
        for index, scan_dir in enumerate(directory.iterdir()):
            ...
            for child in scan_dir.iterdir():
                ...
                if param == 'pi10':
                    pi10 = calc_pi10(tree['wall_global_area'], tree['inner_radius'], save_dir=dirs.out_dir, plot=True, name=param_name)
                    if index == 0:
                    ...
                    df = pd.DataFrame(list(zip(index_list, param_first_list, param_repeat_list)), columns=['ID', 'Pi10_first', 'Pi10_repeat'])
                    df.to_csv(str(Path(outdir / "results_pi10.csv").resolve()))

                elif param == 'WAP':
                    wap_summary = tree.groupby('generation')['wall_global_area_perc'].describe()
                    if index == 0:
                    ...
                    df = pd.DataFrame(list(zip(index_list, param_first_list, param_repeat_list)), columns=['ID', 'WAP_first', 'WAP_repeat'])
                    # wap_summary.to_csv(f'{str(Path(outdir / file_name).resolve())}')
                    df.to_csv(str(Path(outdir / "results_wap.csv").resolve()))
                elif param == 'Ai':
                    ai_summary = tree.groupby('generation')['inner_global_area'].describe()
                    if index == 0:
...
if __name__ == "__main__":
    ...
    aparse.add_argument("--in_pickle", type=str, help="Input path for the airway_summary pickle file.")
    aparse.add_argument("--out_dir", type=str, help="Output folder for the results.", default="/eureka/output")
    aparse.add_argument("--param", type=str, help="Which bronchial parameters to calculate. Options: WA, WAP, Pi10, Ai, Ao, WT")
