function [biomarkers, stats, extras, data] = doIbest_gridsearch (in_folderOpfronted, in_folderVesselsOpfronted, out_folderResults)

    in_logfile = [out_folderResults '/doIBEST.log'];
    diary(in_logfile)
    diary ON
    ...
    if true
        ...
        cfg.files.folderCT              = [baseDataDir '/CTs_ParamTunning/'];
        cfg.files.folderOpfronted       = in_folderOpfronted;
        cfg.files.folderVesselsOpfronted= in_folderVesselsOpfronted;
        cfg.files.subjects_file         = [baseDataDir '/Subjects.csv'];
        cfg.files.saveFileFolder        = [out_folderResults '/MATLAB_pairingResults/'];
        cfg.files.saveFileResultsAirways= [out_folderResults '/IBEST_ResultsPerAirway.csv'];
        cfg.files.saveFileResultsMedian = [out_folderResults '/IBEST_ResultsMedianPerSubject.csv'];
        cfg.files.visualisations        = [out_folderResults '/visualisations/'];
        
        % to measure lung volume for normalisation
        cfg.files.lungSegmentations     = [baseDataDir '/Lungs-bottom-d40-400-800/'];
        cfg.saveFileNameLungVolume      = [out_folderResults '/lungVolumes_Ivacaftor.mat'];        

        % Analyse correlation of manual annotations with computed measurements of segmented airways / vesels
        cfg.matchWithGroundTruth        = true;
        cfg.files.casesWithGroundTruth  = {'b1_03', 'b1_04', 'b1_07', 'b1_17', 'b1_18', 'b1_20', 'b1_23', 'b1_27', 'b1_36', 'b1_39', 'b1_41', 'b1_42', 'b1_45', 'b1_47', 'b2_01', 'b2_02', 'b2_05', 'b2_09', 'b2_16'};
        cfg.files.folderGroundTruth     = [baseDataDir '/AirwaysVessels_Annotations/']
        cfg.files.saveFileResultsAirwaysWithGT = [out_folderResults '/IBEST_ResultsPerAirwayWithGT.csv'];
        cfg.files.saveFileResultsVesselsWithGT = [out_folderResults '/IBEST_ResultsPerVesselWithGT.csv'];
