...
class PhantomTrainer:
    def __init__(self, p_vol: str, p_seg: str, out_dir: str):
        ...
        self.root_out = self.out_dir / self.volume.stem
        self.inner_vol = str(self.root_out / "_surface0.nii.gz")
        self.outer_vol = str(self.root_out / "_surface1.nii.gz")
    ...
    def process_phantom(self, run_number: int,
        ...
        logging.info(f"Starting Phantom {str(self.volume)} Training Run No.{run_number} with parameters '{parameters}'\n"
        ...
        # 3. measure the airways - iterate within the folders generated by split
        logging.debug(f"Measuring results for run {run_number}...")
        subprocess.run([measure_split, str(self.volume), self.inner_vol, self.outer_vol, ])
