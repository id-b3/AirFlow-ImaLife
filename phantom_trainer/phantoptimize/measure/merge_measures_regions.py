...
def main(args):
    ...
    basename_inputdir_region1 = basenamedir(args.input_dir_region1).replace(
        "_region1", ""
    )

    list_in_dirs_regions = list_dirs_dir(
        basedir_inputdir_region1, basename_inputdir_region1 + "*"
    )

    # sort input subdirs from region 1 - 8
    list_in_dirs_regions.sort(
        key=lambda name: get_substring_filename(name, "region[0-9]")
    )
    ...
    with open(out_filename, "w") as fout:
        ...
        for ireg, indir_thisreg in enumerate(list_in_dirs_regions):
            in_filename = join_path_names(
                indir_thisreg, "./Opfront_ResultsPerBranch.csv"
            )
            ...
            try:
                with open(in_filename) as fin:
                    ...
                    first_elem_strdata_new = first_elem_strdata + "_region%d" % (
                        ireg + 1
                    )
                    # add suffix with region number to the first elem 'casename'
                    strdata_infile = strdata_infile.replace(
                        first_elem_strdata, first_elem_strdata_new
                    )
                    fout.write(strdata_infile)

            except IOError as e:
                print(
                    "ERROR. Cannot read data in '%s'. "
                    "Add dummy line in output file and skip this region... "
                    % (indir_thisreg)
                )

                list_dummy_data = (
                    ["COPDGene_Phantom_Qr59_region%d" % (ireg + 1)]
                    + ["000"]
                    + ["0.0"] * 6
                    + ["0"] * 2
                    + [""]
