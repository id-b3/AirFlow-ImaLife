...
def split_seg_reg(in_file: str, in_boxes_file: str, out_dir: str):
    ...
    """
    logging.debug(
        f"Splitting {in_file} using {in_boxes_file}. Output results in {out_dir}"
    )
    ...
    templ_output_filenames = join_path_names(
        out_dir, basename_filenoext(in_file) + "_region-%0.2i.nii.gz"
    )
    ...
    for i, iboundox in enumerate(dict_boundboxes_regions.values()):
        ...
        out_region_image = compute_extended_image(
            out_cropped_image, in_image.shape, iboundox
        )

        out_region_file = templ_output_filenames % (i + 1)
        logging.debug(
            f"Output file: {out_region_file}, with dims: {out_region_image.shape}"
        )

        ImageFileReader.write_image(
            out_region_file, out_region_image, metadata=in_image_metadata
        )
    # endfor
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Split segmentation in the 8 regions in phantom"
    )
