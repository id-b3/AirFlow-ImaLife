...
def merge_branch_reg(
    ...
    for ibranch_file in list_in_branch_files:
        ...
        in_branch_data["index"] = [
            ind + curr_offset_index_branch for ind in in_branch_data["index"]
        ]
        logging.debug(f"Merging branch indexes {in_branch_data['index']}")
        in_branch_data["parent"] = [
            ind + curr_offset_index_branch if ind > 0 else 0
            for ind in in_branch_data["parent"]
        ]
        in_branch_data["children"] = [
            [ind + curr_offset_index_branch for ind in in_children_brh]
    ...
    if is_merge_vols:
        logging.debug(
            f"Merge the labelled volume masks of branches found in {in_dir} and output file {out_vol_file}"
        )

        list_in_volmask_files = list_files_dir(in_dir, "*-branch.nii.gz")
        num_regions = len(list_in_volmask_files)
        logging.debug(
            f"Found {num_regions} files with volume masks of branches: {list_in_volmask_files}"
        )
        ...
        for i, ivolmask_file in enumerate(list_in_volmask_files):
            ...
            in_cropped_volmask = np.where(
                in_cropped_volmask > 0, in_cropped_volmask + curr_offset_index_branch, 0
            )

            out_volmask_all = compute_setpatch_image(
                in_cropped_volmask, out_volmask_all, iboundox
            )

            curr_offset_index_branch += np.max(in_volmask)
        # endfor

        out_image_metadata = ImageFileReader.get_image_metadata_info(
            list_in_volmask_files[0]
        )

        ImageFileReader.write_image(
            out_vol_file, out_volmask_all, metadata=out_image_metadata
        )
...
def main(argmts):
    ...
    merge_branch_reg(
        argmts.in_dir,
        argmts.out_file,
        argmts.is_merge_vols,
        argmts.in_boxes_file,
        argmts.out_vol_file,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Merge branches extracted in the 8 regions in phantom"
    )
    parser.add_argument(
        "-i", "--in_dir", type=str, help="Input dir with branches", required=True
    )
    parser.add_argument("-o", "--out_file", type=str, help="Output file", required=True)
    parser.add_argument(
        "--is_merge_vols",
        type=bool,
        help="merge the volume mask of branches ?",
        default=False,
    )
    parser.add_argument(
        "-ib",
