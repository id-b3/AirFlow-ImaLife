...
def main(infiles):
    ...
    def objective(trial):
        for p in infiles.param.split(','):
            if p == 'o':
                param = trial.suggest_int(f'parameter_o-i', infiles.min, infiles.max, step=infiles.step)
                ...
                param = trial.suggest_float(f'parameter_{p}', infiles.min, infiles.max, step=infiles.step)
                trainer.param[p] = param
        try:
            error_inner, error_outer, error_total = trainer.process_phantom(trial.number)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--param', type=str, help="The parameters to optimise. Split with comma e.g. 'i,I,o,O' ", required=True)
    parser.add_argument('--min', type=float, help="Minimum inner derivative value", default=0)
    parser.add_argument('--max', type=float, help="Maximum inner derivative value", default=10)
    ...
    parser.add_argument('--single', type=bool, help="Use single error measure or separate", default=True)
