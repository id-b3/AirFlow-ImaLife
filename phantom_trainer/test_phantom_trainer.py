...
def main(infiles):
    ...
    def objective(trial):
        in_der = trial.suggest_float(
            "inner_derivative", infiles.inner_min, infiles.inner_max, step=infiles.step
        )
        out_der = trial.suggest_float(
            "outer_derivative", infiles.outer_min, infiles.outer_max, step=infiles.step
        )
        try:
            error_inner, error_outer, error_total = trainer.process_phantom(
                trial.number, i_der=in_der, o_der=out_der, s_pen=infiles.sep
            )
    ...
    study = optuna.create_study(
        study_name=infiles.study_name,
        storage=f"sqlite:///{infiles.study_name}.db",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inner_min", type=float, help="Minimum inner derivative value", default=-0.7
    )
    parser.add_argument(
        "--inner_max", type=float, help="Maximum inner derivative value", default=-0.5
    )
    parser.add_argument(
        "--outer_min", type=float, help="Minimum outer derivative value", default=-0.8
    )
    parser.add_argument(
        "--outer_max", type=float, help="Maximum outer derivative value", default=-0.6
    )
